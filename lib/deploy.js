...
const deployGateway = async (gatewayAdmin) => {
...
const deployNFTFactory = async (gatewayAddress) => {
  ...
  const nftFactory = await NFTFactory.deploy(gatewayAddress);
  await nftFactory.deployed();

  return { nftFactory, libDeployBasicERC721, libDeployBasicERC1155 };
}

const deployERC20Factory = async (gatewayAddress) => {
  const ERC20Factory = await hre.ethers.getContractFactory("ERC20Factory");
  const erc20factory = await ERC20Factory.deploy(gatewayAddress);
